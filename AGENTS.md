...
- 使用 Docker Compose 部署，容器对外绑定端口 `127.0.0.1:3000:3000`，数据目录挂载为 `./data:/app/data`。
- 建议通过 Nginx 等反代对外暴露，配置 HTTPS。
- 线上 `.env` 包含密钥、Cookie 和第三方服务配置；部署代码时不要默认覆盖 `.env`，除非用户明确要求同步配置。
- 线上 `data/` 是运行数据目录，部署、测试或排障时不要删除、重置或覆盖。
- 常规代码更新流程：构建新镜像并重启容器，然后用容器 healthcheck 和 `/api/health` 验证。
- 更新前建议备份服务器现有源码；备份不应包含 `.env` 的明文输出。
